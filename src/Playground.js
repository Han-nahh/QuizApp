...
const Playground = ({route}) => {
    ...
    const getQuestions=async()=>{
        ...
        try {
            const db = firebase.firestore(); // Ensure the Firebase app and Firestore instance are initialized
            const questionsRef = db.collection('questions');
            const snapshot = await questionsRef.where('category', '==', category).get();
        
            if (snapshot.empty) {
              console.log("No matching documents regarding this category");
              return;
            }
        ...
        renderItem={({item,index})=>(
            <View style={styles.questionContainer}>
               ...
               disabled={showResults} >
                ...
                <TouchableOpacity style={[styles.option,
                selectedOptions[index]===5&&styles.selectedOptions,
                showResults&&item.correctOption===5&&styles.correctOption,
                showResults&&selectedOptions[index]===5&&selectedOptions[index]!==item.correctOption && styles.wrongOption]}
               onPress={()=>handleOptionSelect(index,5)}
               disabled={showResults} >
                    <Text>{item.option5}</Text>
